...
class RequestIdFilterTest {
    ...
    void generatesAndPropagatesRequestIdWhenMissing() {
        ...
        assertThat(request.getAttribute(RequestIdFilter.REQUEST_ID_ATTRIBUTE, String.class)).isPresent();
    ...
    void keepsProvidedRequestId() {
        RequestIdFilter filter = new RequestIdFilter();
        MutableHttpRequest<?> request = HttpRequest.GET("/test").header(RequestIdFilter.HEADER_NAME, "provided-id");
        var response = HttpResponse.ok();
        ...
        assertThat(response.getHeaders().get(RequestIdFilter.LEGACY_HEADER_NAME)).isEqualTo("provided-id");
    }

    @Test
    void acceptsLegacyCorrelationHeader() {
        RequestIdFilter filter = new RequestIdFilter();
        MutableHttpRequest<?> request = HttpRequest.GET("/test").header(RequestIdFilter.LEGACY_HEADER_NAME,
                "legacy-id");
        var response = HttpResponse.ok();
