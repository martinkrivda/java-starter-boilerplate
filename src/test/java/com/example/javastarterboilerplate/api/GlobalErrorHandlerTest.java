...
class GlobalErrorHandlerTest {
    ...
    void buildsNotFoundMalformedAndUnexpectedErrors() {
        GlobalErrorHandler handler = new GlobalErrorHandler(factory("https://api.example.com/problems"));
        MutableHttpRequest<?> request = request("request-1", "/api/test");

        ApiResponse<Void> notFound = handler.notFound(request).body();
        ApiResponse<Void> malformed = handler.malformedJson(request, new SerdeException("bad json")).body();
        ApiResponse<Void> unexpected = handler.unexpected(request, new IllegalStateException("boom")).body();

        assertThat(notFound.error().type()).isEqualTo("https://api.example.com/problems/resource-not-found");
        assertThat(notFound.error().code()).isEqualTo("E1004");
        assertThat(malformed.error().type()).isEqualTo("https://api.example.com/problems/malformed-request");
        assertThat(malformed.error().code()).isEqualTo("E1001");
        assertThat(unexpected.error().type()).isEqualTo("https://api.example.com/problems/internal-error");
        assertThat(unexpected.error().detail()).isEqualTo("An internal error occurred. Please try again later.");
    }
    ...
    void mapsHttpStatusExceptionsToProblemDetails() {
        ...
        ApiResponse<Void> badRequest = handler
                .httpStatus(request, new HttpStatusException(HttpStatus.BAD_REQUEST, "bad request")).body();
        ApiResponse<Void> unsupported = handler
                .httpStatus(request, new HttpStatusException(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "unsupported media"))
                .body();
        ApiResponse<Void> generic = handler
                .httpStatus(request, new HttpStatusException(HttpStatus.TOO_MANY_REQUESTS, "slow down")).body();
        ApiResponse<Void> server = handler
                .httpStatus(request, new HttpStatusException(HttpStatus.SERVICE_UNAVAILABLE, "dependency down")).body();
        ...
        assertThat(unsupported.error().type()).isEqualTo("https://api.example.com/problems/unsupported-media-type");
        ...
        assertThat(server.error().detail()).isEqualTo("An internal error occurred. Please try again later.");
    }
    ...
    void convertsConstraintViolationsToJsonPointerErrors() {
        ...
        ConstraintViolationException exception = new ConstraintViolationException(
                Set.of(violation("create.request.items[0].name", NotBlank.class, "must not be blank"),
                        violation("create.request.items[1].code", Pattern.class, "must match"),
                        violation("create.request.items[0].quantity", Positive.class, "must be greater than 0"),
        ...
        assertThat(response.error().type()).isEqualTo("https://api.example.com/problems/validation-error");
        assertThat(response.error().errors()).extracting("pointer").contains("/items/0/name", "/items/1/code",
                "/items/0/quantity", "/title");
        assertThat(response.error().errors()).extracting("code").contains("REQUIRED", "INVALID_FORMAT", "MIN_VALUE",
                "INVALID_LENGTH");
    ...
    void handlesUnknownConstraintCodesAndBlankPointers() {
        ...
        assertThat(response.error().errors()).singleElement().satisfies(item -> {
    ...
    private ConstraintViolation<Object> violation(String propertyPath, Class<? extends Annotation> annotationType,
            String message) {
