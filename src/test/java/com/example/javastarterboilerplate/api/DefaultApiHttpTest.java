...
class DefaultApiHttpTest {
    ...
    void startsWithoutPersistenceAndServesInfoAndSampleEndpoints() {
        HttpResponse<String> infoResponse = httpClient.toBlocking().exchange(HttpRequest.GET("/rest/v1/info"),
                String.class);
        ...
        assertThat(documents).extracting(document -> document.get("name")).contains("Seeded in-memory sample");
