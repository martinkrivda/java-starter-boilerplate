...
class SampleDocumentPersistenceAdapterTest {
    ...
    @Inject
    SampleDocumentRepository sampleDocumentRepository;

    @Test
    void loadsSeededDataFromFlywayMigration() {
        assertThat(sampleDocumentRepository.findAll()).extracting(SampleDocument::name).contains("Seeded H2 sample");
    }

    @Test
    void savesAndReadsDomainDocuments() {
        SampleDocument sampleDocument = new SampleDocument(UUID.randomUUID(), "Persisted sample",
                "archive/persisted-sample.pdf", Instant.parse("2026-04-13T12:34:56Z"));
