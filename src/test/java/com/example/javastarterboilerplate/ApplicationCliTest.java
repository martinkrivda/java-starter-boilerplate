...
class ApplicationCliTest {
    ...
    void startsServerWithRemainingArgumentsForServeCommand() {
        CliCommandResult result = ApplicationCli.handle(new String[]{"serve", "--port=8081"}, outputStream(),
                outputStream());
    ...
    void printsHelp() {
        ...
        CliCommandResult result = ApplicationCli.handle(new String[]{"help"}, stream(stdout), outputStream());
    ...
    void printsVersionFromSystemPropertyWhenAvailable() {
        ...
        CliCommandResult result = ApplicationCli.handle(new String[]{"version"}, stream(stdout), outputStream());
    ...
    void printsEnvironmentDetails() {
        ...
        CliCommandResult result = ApplicationCli.handle(new String[]{"env"}, stream(stdout), outputStream());
    ...
    void rejectsUnknownCommands() {
        ...
        CliCommandResult result = ApplicationCli.handle(new String[]{"unknown"}, outputStream(), stream(stderr));
