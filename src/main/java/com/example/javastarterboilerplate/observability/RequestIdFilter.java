...
public class RequestIdFilter {
    ...
    public void onRequest(MutableHttpRequest<?> request) {
        String requestId = request.getHeaders().get(HEADER_NAME, String.class).orElseGet(() -> request.getHeaders()
                .get(LEGACY_HEADER_NAME, String.class).orElseGet(() -> UUID.randomUUID().toString()));
