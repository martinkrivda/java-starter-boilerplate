...
public class GlobalErrorHandler {
    ...
    public io.micronaut.http.HttpResponse<ApiResponse<Void>> notFound(HttpRequest<?> request) {
        return responseFactory.error(request, HttpStatus.NOT_FOUND, "resource-not-found", "Resource not found",
                "Requested resource was not found.", "E1004", null);
    }

    @Error(global = true, exception = HttpStatusException.class)
    public io.micronaut.http.HttpResponse<ApiResponse<Void>> httpStatus(HttpRequest<?> request,
            HttpStatusException exception) {
        HttpStatus status = exception.getStatus();
        return responseFactory.error(request, status, typeSlug(status), title(status), safeDetail(status, exception),
                code(status), null);
    }

    @Error(global = true, exception = ConstraintViolationException.class)
    public io.micronaut.http.HttpResponse<ApiResponse<Void>> validation(HttpRequest<?> request,
            ConstraintViolationException exception) {
        List<ApiFieldError> errors = exception.getConstraintViolations().stream().map(this::toFieldError).toList();
        return responseFactory.error(request, HttpStatus.UNPROCESSABLE_ENTITY, "validation-error", "Validation error",
                "Request is not valid.", "E1003", errors);
    }

    @Error(global = true, exception = SerdeException.class)
    public io.micronaut.http.HttpResponse<ApiResponse<Void>> malformedJson(HttpRequest<?> request,
            SerdeException exception) {
        return responseFactory.error(request, HttpStatus.BAD_REQUEST, "malformed-request", "Bad request",
                "Request body is not a valid JSON document.", "E1001", null);
    }

    @Error(global = true, exception = Throwable.class)
    public io.micronaut.http.HttpResponse<ApiResponse<Void>> unexpected(HttpRequest<?> request, Throwable exception) {
        return responseFactory.error(request, HttpStatus.INTERNAL_SERVER_ERROR, "internal-error",
                "Internal server error", "An internal error occurred. Please try again later.", "E3001", null);
    }

    private ApiFieldError toFieldError(ConstraintViolation<?> violation) {
        String pointer = toJsonPointer(violation);
        return new ApiFieldError(pointer, fieldName(pointer), validationCode(violation), violation.getMessage());
    }
    ...
    private String toJsonPointer(ConstraintViolation<?> violation) {
        ...
        return rawSegments.subList(startIndex, rawSegments.size()).stream()
                .flatMap(segment -> expandSegment(segment).stream()).map(this::escapeJsonPointerSegment)
    ...
    private String validationCode(ConstraintViolation<?> violation) {
        ...
        if ("Min".equals(simpleName) || "Positive".equals(simpleName) || "PositiveOrZero".equals(simpleName)) {
            return "MIN_VALUE";
        }
