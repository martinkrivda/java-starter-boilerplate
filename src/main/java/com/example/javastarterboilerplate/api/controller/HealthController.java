...
public class HealthController {
  ...
  @Operation(
      ...
      description = "Returns service status, version, uptime and dependency checks.")
  public HealthFullResponse health() {
    List<HealthCheckResponse> checks = checks();
    return new HealthFullResponse(
        applicationShutdownState.isShuttingDown() ? "DOWN" : "UP",
        applicationInfoService.getInfo().version(),
        uptimeSeconds(),
        checks);
  }
            ...
            integration ->
                checks.add(
                    new HealthCheckResponse(
                        ...
                        integration.enabled() ? "UP" : "DOWN",
                        1L,
                        integration.detail(),
                        Map.of("enabled", integration.enabled()))));
