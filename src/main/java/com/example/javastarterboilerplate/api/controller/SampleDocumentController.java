...
public class SampleDocumentController {
    ...
    public SampleDocumentController(SampleDocumentService sampleDocumentService, ApiResponseFactory responseFactory) {
        this.sampleDocumentService = sampleDocumentService;
        this.responseFactory = responseFactory;
    }

    @Get
    @Operation(summary = "List sample documents", description = "Returns boilerplate sample document records from the persistence adapter.")
    public ApiResponse<List<SampleDocumentResponse>> list(HttpRequest<?> request) {
        return responseFactory.success(request,
                sampleDocumentService.findAll().stream().map(this::toResponse).toList());
    }

    @Get("/{id}")
    @Operation(summary = "Get a sample document", description = "Loads a single boilerplate sample document by identifier.")
    public ApiResponse<SampleDocumentResponse> get(HttpRequest<?> request, UUID id) {
        SampleDocumentResponse response = sampleDocumentService.findById(id).map(this::toResponse)
                .orElseThrow(() -> new HttpStatusException(io.micronaut.http.HttpStatus.NOT_FOUND,
                        "Requested resource was not found."));
        return responseFactory.success(request, response);
    }

    @Post
    @Operation(summary = "Create a sample document", description = "Creates a boilerplate sample record without applying any PDF sealing or signature workflow.")
    public HttpResponse<ApiResponse<SampleDocumentResponse>> create(HttpRequest<?> httpRequest,
            @Body @Valid CreateSampleDocumentRequest request) {
        SampleDocument created = sampleDocumentService.create(request.name(), request.storageKey());
        return responseFactory.created(httpRequest, toResponse(created),
    ...
    private SampleDocumentResponse toResponse(SampleDocument sampleDocument) {
        return new SampleDocumentResponse(sampleDocument.id(), sampleDocument.name(), sampleDocument.storageKey(),
