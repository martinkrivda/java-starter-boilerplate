...
public class ApiResponseFactory {
    ...
    public <T> MutableHttpResponse<ApiResponse<T>> created(HttpRequest<?> request, T data, URI location) {
        return HttpResponse.created(success(request, data)).headers(headers -> headers.location(location));
    }

    public MutableHttpResponse<ApiResponse<Void>> error(HttpRequest<?> request, HttpStatus status, String typeSlug,
            String title, String detail, String code, List<ApiFieldError> errors) {
        ResponseMeta meta = meta(request);
        ApiProblemDetails problem = new ApiProblemDetails(problemType(typeSlug), title, status.getCode(), detail,
                request.getPath(), code, meta.requestId(), errors == null || errors.isEmpty() ? null : errors);
        return HttpResponse.<ApiResponse<Void>>status(status).body(ApiResponse.<Void>failure(problem, meta))
                .contentType("application/json").header(HttpHeaders.CACHE_CONTROL, "no-store");
    }
    ...
    private String requestId(HttpRequest<?> request) {
        return request.getAttribute(RequestIdFilter.REQUEST_ID_ATTRIBUTE, String.class).orElse("not-set");
    }
