...
public class S3ObjectStorageAdapter implements ObjectStorage {
    ...
    public StoredObject download(String key) {
        ResponseBytes<GetObjectResponse> response = s3Client
                .getObjectAsBytes(GetObjectRequest.builder().bucket(properties.getBucket()).key(key).build());

        Map<String, String> metadata = response.response().metadata();

        return new StoredObject(key, response.response().contentType(), response.asByteArray(), metadata);
    }

    @Override
    public StoredObject upload(StoredObjectUpload upload) {
        s3Client.putObject(
                PutObjectRequest.builder().bucket(properties.getBucket()).key(upload.key())
                        .contentType(upload.contentType()).metadata(upload.metadata()).build(),
